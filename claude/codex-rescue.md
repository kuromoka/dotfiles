...
## 600 秒タイムアウトで結果が宙に浮く問題（必ず対処する）

Codex のタスクが 600 秒を超えると Bash が自動でバックグラウンドへ移す。このとき
**`codex-rescue` サブエージェント自身はバックグラウンドジョブの状態確認・出力取得の権限を持たない**ため、
「実行中か終了済みか分からない」と報告して停止する。**実際には完了していて出力ファイルに結果が
書かれているのに、誰も取りに行かない**という状態になる（2026-07-28 に実際に発生）。

**メインモデル側の対処（毎回やる）**:

1. サブエージェントを起動したら、**出力ファイルのパスを控える**。サブエージェントは停止時の報告に
   `/private/tmp/.../tasks/<id>.output` の形でパスを載せてくる。
2. サブエージェントが「バックグラウンドに移行した／状態が分からない」と報告したら、
   **メインモデルが直接そのファイルを読む**（`cat`）。これは Bash background task の出力であって
   サブエージェントの JSONL トランスクリプトではないので、読んで問題ない。
3. ファイル末尾に Codex の最終出力（`[codex] Turn completed.` の後）が入っている。
   **完了していればそれをそのままユーザーへ返す**。
4. まだ走っているなら `pgrep -f codex` とファイルの更新時刻で判断し、`Monitor` か
   `until` ループで完了を待つ。**サブエージェントに状態確認を依頼しても答えられない**ので無駄。

**予防**: 長時間化が見込まれるタスク（大きなファイルを読ませる等）は、最初から
Agent の `run_in_background: true` で投げ、上記 2〜3 の手順を前提に動く。

## 出力の検証について（改変禁止と混同しない）

「Codex の stdout を改変・要約しない」は**そのまま提示する**という意味であって、
**内容を検証してはいけないという意味ではない**。Codex はドメイン知識の誤りを含むことがある
（2026-07-28: ポケカのリプレイ分析で「相手バトル場にダメカンを置くべきだった」と指摘したが、
そのカードの特性は相手ベンチしか対象にできず、**選択肢として存在しない手**だった）。
