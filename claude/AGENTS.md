...
- ユーザー本人の名義で公開・送信する日本語文章の作成・推敲では、利用可能な場合は `kuromoka-writing` スキルを適用する。
...
- 設計、進行方向、合否基準、統合判断はメインエージェントが担当する。
- 複数ファイルにまたがるコード変更または作業量が支配的な機械的実装は、利用可能な場合は`~/.codex/agents/implementer.toml`のエージェントへ委任する。
- 複数ファイルの読み込みが中心となる調査、検索、監査は、利用可能な場合は`~/.codex/agents/researcher.toml`のエージェントへ委任する。
- 最終検証にはカスタムエージェントを使わず、フレッシュなcontextの別エージェントを使う。
- 必要なカスタムエージェントを起動できない場合は、理由をユーザーへ報告してからメインエージェントがフォールバックする。
