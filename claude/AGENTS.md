...
「ループエンジニアリング」= 指示役を自分から降ろし、エージェントが目標達成まで自動で反復するシステムを設計すること（prompt < context < harness < **loop** の4層目）。長時間・ハンズオフで回すための原則。

## 検証ループを閉じる（最重要）

- タスク着手時に **機械可読な合否チェック**（テスト / 型チェック / lint / ビルドの exit code / 差分・スクショ比較など）を1つ決め、それが緑になるまで自走で反復する。
- チェックが無いと「done に見える」が唯一の停止信号になり、ユーザーが検証ループの役を負わされる。**先にチェックを定義してから**着手する。

## ガードレール（自走でも必ず）

- **最大ループ回数**を決める（例: 5周）。
- **同一エラーが2回連続**したら止めてエスカレーションする（無限ループ・空回り防止）。
- **トークン / 予算のハード上限**を必ず設ける（暴走による予想外の課金を防ぐ）。
- **判断は手放さない**: 実行は任せても「どの方向で進めるか・何を良しとするか」は人間／メインモデルが決める。

## 早すぎる完了の防止

- **テストの削除・改変でチェックを通すのは禁止**。要件の取りこぼし・隠れバグの原因になる。
- 「passing」にするのは **実際のテストツールで自己検証してから**。可能なら実環境での確認（ブラウザ操作等）も行う。

## 評価役の分離

- 自己採点は甘くなる（書いた本人は誤字に気づけない）。**検証はフレッシュな context の別エージェント**に任せる（差分と判定基準だけ渡し、生成時の思考は見せない）。

## context と記憶

- 恒久ルールは `CLAUDE.md` / `AGENTS.md` に置く（**毎リクエスト再注入**されるため。初回プロンプトに書くと compaction で失われる）。
- compaction 時に保持すべきもの: **変更したファイル一覧 / 検証・テストコマンド / 現在のタスク状態 / 未解決の問題**。
- **compaction だけでは状態を保てない**（各セッションは記憶ゼロで始まる）。長時間・複数セッションのタスクは、進捗ログ・`feature_list.json`（boolean `passes`）・git 履歴など **外部の永続アーティファクト**で橋渡しする。

> 出典: Anthropic「Claude Code best practices」「effective context engineering / harnesses for long-running agents」, Claude Agent SDK docs（一次ソースで 3-0 検証済み）。Addy Osmani によるループエンジニアリングの定義に基づく。
